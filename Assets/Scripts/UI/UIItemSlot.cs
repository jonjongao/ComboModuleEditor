...
namespace UnityEngine.UI
{
    ...
    public class UIItemSlot : UISlotBase
    {
        ...
        [SerializeField]
        private UIItemSlot_Group m_SlotGroup = UIItemSlot_Group.None;
        [SerializeField]
        private int m_ID = 0;
        ...
        public override bool PerformSlotSwap(Object sourceObject)
        {
            ...
            if (sourceSlot is UIItemSlot)
            {
                ...
                Debug.Log("Original is:" + sourceItemInfo.Name + "/Dmg:" + sourceItemInfo.Damage);
                // Assign the source slot by this one
                //assign1 = (sourceSlot as UIItemSlot).Assign(this.GetItemInfo());
                Debug.Log("Item about to swap:" + this.GetItemInfo().Name + "/Dmg:" + this.GetItemInfo().Damage);
                if (this.GetItemInfo().Name == sourceItemInfo.Name)
                {
                    Debug.LogWarning("Same item!");
                    /*
                     * 合成武器
                     */
                    UIItemInfo craft = new UIItemInfo();
                    craft.ID = this.GetItemInfo().ID;
                    craft.Damage = this.GetItemInfo().Damage;
                    craft.EquipType = this.GetItemInfo().EquipType;
                    craft.Icon = this.GetItemInfo().Icon;
                    craft.ItemType = this.GetItemInfo().ItemType;
                    craft.Name = this.GetItemInfo().Name;
                    craft.prefab = this.GetItemInfo().prefab;
                    int newDmg = craft.Damage + sourceItemInfo.Damage;
                    craft.Damage = newDmg;
                    Debug.LogWarning("Craft done! New item damage is:" + newDmg);
                    assign1 = (sourceSlot as UIItemSlot).Assign(craft);
                    //assign2 = false;
                    this.Unassign();
                    assign2 = true;
                }
            ...
            else if (sourceSlot is UIEquipSlot)
            {
                ...
                assign2 = this.Assign(sourceItemInfo);
            }
